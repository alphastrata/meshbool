...
/// A trait for safe conversions between integer types that may involve truncation
/// This replaces C++-style implicit conversions with explicit, safe alternatives
pub trait SafeFrom<T: Copy> {
    fn safe_from(other: T) -> Self;
}

/// A trait for safe conversions to other types that may involve truncation
pub trait SafeInto<T: Copy> {
    fn safe_into(self) -> T;
}
...
impl SafeFrom<i32> for usize {
    fn safe_from(other: i32) -> Self {
        // This is safe in practice for our use cases where indices are non-negative
        // and within reasonable bounds for mesh operations
        other as usize
    }
}

impl SafeFrom<usize> for usize {
    fn safe_from(other: usize) -> Self {
        other
    }
}
