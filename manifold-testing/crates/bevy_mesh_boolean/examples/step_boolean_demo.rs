...
use bevy_step_loader::load_step_file_from_path;
...
fn setup_demo(
    ...
    let step_handle = if let Some(step_file_path) = &cli_args.step_file {
        ...
        // Use the direct path loading function instead of asset server
        match load_step_file_from_path(step_file_path) {
            Ok(step_asset) => {
                ...
                log::debug!("Loaded STEP mesh with {} vertices", vertex_count);
                // Add the mesh to the assets and return the handle
                meshes.add(step_asset.mesh)
            },
            Err(e) => {
                log::error!("Failed to load STEP file from path {:?}: {}", step_file_path, e);
                // Fallback to cube
    ...
    let secondary_shape = commands
        .spawn(PbrBundle {
            ...
            transform: Transform::from_xyz(0.5, 0.0, 0.0), // Overlap with the first shape
            ..default()
        })
...
// This system exits the app when 'q' is pressed
fn exit_on_q_key(keys: Res<ButtonInput<KeyCode>>, mut exit: EventWriter<AppExit>) {
