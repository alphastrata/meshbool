    ...
    match (primary_manifold_opt, secondary_manifold_opt) {
        (Some(mut primary_manifold), Some(mut secondary_manifold)) => {
            ...
            if result_vertices == 0 {
                eprintln!("[TIMING {}] [WARNING] Result mesh has 0 vertices - boolean operation may have failed", 
                         operation_start_time.elapsed().as_micros());
            } else {
        ...
        _ => {
            ...
            // Check if the result is empty
            if result_vertices == 0 {
                eprintln!("[TIMING {}] [WARNING FALLBACK] Result mesh has 0 vertices - boolean operation may have failed", 
                         operation_start_time.elapsed().as_micros());
            } else {
