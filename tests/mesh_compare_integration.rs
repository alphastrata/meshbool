//! Integration tests comparing meshbool implementation with manifold-rs reference implementation
//!
//! These tests verify that our meshbool implementation produces results that are
...
macro_rules! approx_eq {
    ($our_mesh:expr, $their_mesh:expr) => {
        approx_eq_meshes($our_mesh, $their_mesh, None)
    };
    ($our_mesh:expr, $their_mesh:expr, $tolerance:expr) => {
        approx_eq_meshes($our_mesh, $their_mesh, Some($tolerance))
    };
}
