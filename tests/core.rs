use meshbool::{cube, get_mesh_gl, cylinder, translate, rotate};
...
fn test_transformations() {
    ...
    assert_eq!(original_mesh.tri_verts.len(), translated_mesh.tri_verts.len());
...
fn test_mesh_properties() {
    ...
    assert!(mesh.vert_properties.len() as u32 >= mesh.num_prop * (mesh.vert_properties.len() as u32 / mesh.num_prop)); // Basic consistency check
}
